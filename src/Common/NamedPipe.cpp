...
void PipeChannel::ensureServerReaderClosed()
{
  if(!serverReader_)
    return;

  serverReader_->close();
  delete serverReader_;
  serverReader_ = NULL;
}

void PipeChannel::ensureServerReaderOpened()
{
  if(serverReader_)
    return;

  serverReader_ = new NamedPipeReader(pipeClientServer_);
